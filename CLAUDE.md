...
**Phase 3 (handoff) underway** — see `allard-prize-phase3-handoff-spec.md`. The conversational-maintenance tool is **OpenAI Codex** (Preet already has ChatGPT Plus). **Phase 3A — done except branch protection:** the `AGENTS.md` runbook (done), the `/admin/maintenance` launch page with jargon-shielded Approach A (done), and **migrate-on-deploy live** — `vercel.json`'s `buildCommand` runs `pnpm db:migrate:deploy`, preview deploys migrate their own auto-branched Neon DB, prod migrates on merge (see the Database & migrations section of `AGENTS.md`). **Branch protection on `main` is deferred until Phase 2 cutover is complete** (it would otherwise force Brian through PRs for remaining 2F work).
